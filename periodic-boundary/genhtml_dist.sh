#!/bin/bash
# Generate html files for periodic boundary changing based on distance.
...
make_index() {
  cd $OUT_PBD
  local dest=$OUT_PBD/index.html
  echo "<h1>SPS-P Model Simulation: Periodic Boundary Changing Based on Distance</h1><ul>" > $dest
